...
func main() {
	...
	if *toAddrHex == "" || *amountEth <= 0 {
		log.Fatalf("send mode requires --to and --amount flags")
	}
...
func sendTransaction(toAddrHex string, amountEth float64) {
	...
	privateHex := os.Getenv("SENDER_PRIVATE_KEY")
	if privateHex == "" {
		log.Fatalf("SENDER_PRIVATE_KEY is not set (required for send mode)")
	}
	...
	// 解析私钥
	privateKey, err := crypto.HexToECDSA(trim0x(privateHex))
	if err != nil {
	...
	if !ok {
		log.Fatal("errpr casting public key to ECDSA")
	}
	...
	// 获取建议的 Gas 价格（使用 EIP-1559 动态费用）ww
